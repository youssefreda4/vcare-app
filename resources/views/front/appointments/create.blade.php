    ...
    <div class="container">
        ...
        <div class="d-flex flex-column gap-3 details-card doctor-details">
            <div class="details d-flex gap-2 align-items-center">
                <img src="{{ asset('uploads/doctors/' . $doctor->image) }}" alt="doctor" class="img-fluid rounded-circle" height="150"
                    width="150" />
                <div class="details-info d-flex flex-column gap-3">
                    <h4 class="card-title fw-bold">{{ $doctor->name }}</h4>
                    <h6 class="card-title fw-bold">
                        doctor major : {{ $doctor->major->name }}
                    </h6>
                </div>
            </div>
            <hr />
            <x-error></x-error>
            <x-success></x-success>
            <form class="form" action="{{ route('appointments.store',$doctor->id) }}" method="POST" novalidate>
